...
const HelpSupport: React.FC = () => {
    ...
    const [newTicket, setNewTicket] = useState({ subject: '', description: '', priority: 'medium', file: null as File | null });
    ...
    const raiseMutation = useMutation({
        ...
        onSuccess: () => {
            toast.success('Your support ticket has been submitted successfully. Our team will get back to you shortly.');
            setIsRaiseModalOpen(false);
            setNewTicket({ subject: '', description: '', priority: 'medium', file: null });
    ...
    const handleRaiseSubmit = (e: React.FormEvent) => {
        ...
        if (!newTicket.subject.trim()) {
            errors.subject = 'Subject cannot be empty or only spaces';
            hasError = true;
        } else if (newTicket.subject.split(/\s+/).some(word => word.length > 50)) {
            errors.subject = 'Subject contains a word longer than 50 characters';
            hasError = true;
        }

        if (!newTicket.description.trim()) {
            errors.description = 'Description cannot be empty or only spaces';
            hasError = true;
        } else if (newTicket.description.length > 500) {
            errors.description = 'Description must be less than 500 characters';
            hasError = true;
        } else if (newTicket.description.split(/\s+/).some(word => word.length > 50)) {
            errors.description = 'Description contains a word longer than 50 characters';
            hasError = true;
        }
        ...
        raiseMutation.mutate(newTicket);
    };
    ...
    const handleFileChange = (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        if (file && file.size > 5 * 1024 * 1024) { // 5MB limit
            ...
            e.target.value = '';
            setNewTicket({ ...newTicket, file: null });
            return;
        }
    ...
    return (
        <div className="max-w-[1200px] mx-auto font-['AeonikRegular'] relative pb-20 py-10 lg:py-18">
            {/* Header */}
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-10">
                <div>
                    <h1 className="headings-web-h4-headline text-slate-900">Help & Support</h1>
                    <p className="body-text-body-2 text-slate-500 mt-1">Get assistance from our support team</p>
                </div>
                <div className="flex gap-4">
                    ...
                    <button
                        ...
                        className="btn flex items-center gap-2 px-6 !w-auto"
                    >
            ...
            {activeTab === 'faq' ? (
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                    <div className="lg:col-span-2 space-y-4">
                        {isFaqsLoading ? (
                            <div className="flex justify-center p-10 text-slate-500">Loading FAQs...</div>
                            ...
                            <div className="text-center p-10">
                                ...
                                <p className="text-slate-400 text-sm">No FAQs available at the moment</p>
                            </div>
                        )}
                    </div>
                    <div className="lg:col-span-1">
                        <div className="bg-gray-400 rounded-2xl p-8 text-black">
                            <h3 className="headings-web-h6-headline mb-4">Need more help?</h3>
                            <p className="text-slate-400 text-sm mb-8 leading-relaxed font-['AeonikMedium']">Our dedicated support team is available 24/7 to assist you with any questions or issues.</p>
                            <div className="space-y-6">
                                <div className="flex items-center gap-4">
                                    ...
                                    <div>
                                        <p className="text-[10px] uppercase tracking-widest text-slate-500 font-['AeonikBold']">Email us at</p>
                                        <p className="text-sm font-['AeonikBold']">support@smarthealth.com</p>
                                    </div>
                                </div>
                                <div className="flex items-center gap-4">
                                    <div className="w-10 h-10 rounded-xl bg-white/10 flex items-center justify-center text-white">
                                    ...
                                    <div>
                                        <p className="text-[10px] uppercase tracking-widest text-slate-500 font-['AeonikBold']">Response Time</p>
                                        <p className="text-sm font-['AeonikBold']">Under 2 hours</p>
                                    </div>
                                </div>
                            </div>
                        </div>
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 h-[calc(100vh-250px)]">
                    ...
                    <div className="lg:col-span-1 bg-white rounded-lg border border-slate-100 shadow-sm flex flex-col h-full overflow-hidden">
                        <div className="p-4 border-b border-slate-50 bg-slate-50/30">
                            <div className="relative">
                                ...
                                <input
                                    ...
                                    className="w-full pl-10 pr-4 py-2 bg-white text-black border border-slate-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-purple-100"
                                />
                            </div>
                        </div>
                        ...
                        <div className="flex-1 overflow-y-auto">
                            {isTicketsLoading ? (
                                <div className="flex justify-center p-10 text-slate-500">Loading tickets...</div>
                                    ...
                                    {ticketsData?.map((ticket: any) => (
                                        ...
                                        >
                                            ...
                                            <div className="flex-1 overflow-hidden">
                                                ...
                                                <p className="text-slate-500 text-[12px] truncate break-words mt-1">{ticket.description}</p>
                    ...
                    <div className="lg:col-span-2 bg-white rounded-lg border border-slate-100 shadow-sm flex flex-col h-full overflow-hidden relative">
                        {!selectedTicketId ? (
                            ...
                            <div className="flex-1 flex items-center justify-center text-slate-500">Loading conversation...</div>
                        ) : (
                            <>
                                ...
                                <div className="flex-1 overflow-y-auto p-6 space-y-6">
                                    ...
                                    <div className="flex gap-4">
                                        ...
                                        <div className="bg-slate-50 rounded-2xl rounded-tl-none p-4 max-w-[80%]">
                                            <p className="text-slate-900 text-sm leading-relaxed break-words">{ticketDetails.description}</p>
                                        ...
                                        return (
                                            <div key={idx} className={`flex gap-4 ${isAdmin ? '' : 'flex-row-reverse'}`}>
                                                <div className={`w-10 h-10 rounded-full flex items-center justify-center flex-shrink-0 ${isAdmin ? 'bg-purple-100' : 'bg-slate-100'}`}>
                                                    {isAdmin ? <MessageSquare size={20} className="text-purple-500" /> : <MessageSquare size={20} className="text-slate-400" />}
                                                </div>
                                                <div className={`${isAdmin ? 'bg-purple-50 text-slate-900 rounded-tl-none' : 'bg-slate-50 text-slate-900 rounded-tr-none'} rounded-2xl p-4 max-w-[80%]`}>
                                    ...
                                    {ticketDetails.status === 'closed' ? (
                                        ...
                                        <form onSubmit={handleResponseSubmit} className="relative">
                                            <textarea
                                                ...
                                                className="w-full pr-14 pl-4 py-4 text-black bg-slate-50 border-none outline-none rounded-2xl text-sm focus:ring-2 focus:ring-purple-100 min-h-[80px] resize-none"
                                                maxLength={500}
                                            />
                                            <button
                                                type="submit"
                                                disabled={respondMutation.isPending || !responseMessage.trim()}
                                                className="absolute right-3 bottom-3 text-black p-3 bg-slate-900 text-white rounded-xl hover:bg-slate-800 transition-colors disabled:opacity-50 disabled:cursor-not-allowed shadow-lg"
                                            >
                                                {respondMutation.isPending ? <Clock size={20} className="animate-spin" /> : <Send size={20} />}
                                            </button>
                                        </form>
                                    )}
                                </div>
                            ...
                            <form onSubmit={handleRaiseSubmit} className="p-6 space-y-5">
                                    ...
                                    <div className="grid grid-cols-3 gap-3 mt-2">
                                        {['low', 'medium', 'high'].map((p) => (
                                            <button
                                                ...
                                                className={`py-2 rounded-xl text-[12px] font-['AeonikBold'] uppercase tracking-widest border transition-all ${newTicket.priority === p ? 'bg-slate-900 text-white border-slate-900' : 'bg-slate-50 text-slate-500 border-slate-100 hover:border-slate-300'}`}
                                            >
                                                {p}
                                            </button>
                                        ))}
                                ...
                                <div className="form-group">
                                    ...
                                    <textarea
                                        ...
                                        className={`form-control h-32 py-4 resize-none ${formErrors.description ? 'border-red-500' : ''}`}
                                        maxLength={500}
                                    />
                                    {formErrors.description && <p className="text-red-500 text-xs mt-1">{formErrors.description}</p>}
                                </div>
                                <div className="form-group">
                                    <label className="text-sm font-['AeonikBold'] text-slate-800 mb-1 block">Attachment (Optional)</label>
                                    <label className="form-control flex items-center justify-between cursor-pointer hover:bg-slate-50 transition-colors">
                                        <span className="text-slate-400 text-[14px]">
                                            {newTicket.file ? newTicket.file.name : 'Upload image or PDF'}
                                        </span>
                                ...
                                <button
                                    ...
                                    className="w-full btn flex items-center justify-center gap-2 py-4"
                                >
                                    {raiseMutation.isPending ? <>Submitting...</> : <><Plus size={20} /> Raise Ticket</>}
                                </button>
                            </form>
                        </div>
                    </div>
